...
export function generateStaticParams() {
  return getAllTaskMeta().map((t) => ({ slug: t.slug }));
}
...
export async function generateMetadata({
  ...
  const task = getTaskContent(slug);
  if (!task) return { title: "Задача не найдена" };
  return {
    title: `${task.title} — задача ${task.num}`,
    description: `Задача ${task.num} по конкурентности Go: ${task.title}.`,
  };
}
...
export default async function TaskPage({
  ...
  // Render MDX on the server, pass rendered nodes to the client workspace.
